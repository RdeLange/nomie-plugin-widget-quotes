<script>
  ...
  const plugin = new NomiePlugin({
        name: pluginname,
        emoji: pluginemoji,
        description: "Nomie Quotes Widgets Plugin",
        uses: ["searchNotes"],
        version: "0.1",
  ...
  function loadInitParams() {
    ...
    plugin.onRegistered(async () => {
      ...
      Widgets = plugin.storage.getItem('widgets') || [{"widgetid":"1", "name":"Stoic-en","emoji":"🏛️","color":"#C47ADA","config":{"quotetheme":"stoic-en"}},{"widgetid":"2", "name":"Stoic-nl","emoji":"🏛️","color":"#C47ADA","config":{"quotetheme":"stoic-nl"}}];
      WidgetsEdit = Widgets;
      if (Widgets.length == 2){
...
function showMain(){
 ...
 const addByTemplate = (event) => {
  const widget = {
        ...
        config: {"quotetheme":"custom","quotes":event.detail.detail.template.quotes}
			};
...
const deletewidget = async event => {
    let res = await plugin.confirm('Delete Quotes Widget Config?', 'Are you sure you want to delete this Quote Widget configuration?');
if(res.value) {
...
const editMode = (event) => {
  const widgetId=event.detail;
    ...
    lastquotes = Widgets[widget2edit].config.quotes;
    isEditMode = true;
    view="mainedit";
}
...
const backtolastsave = () => {
  const lastsave = {"widgetid":Widgets[widget2edit].widgetid,"name":lastname,"description":lastdescription,"color":lastcolor,"emoji":lastemoji,"config":{"quotetheme":"custom","quotes":lastquotes}}
